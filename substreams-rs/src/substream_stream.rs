use crate::pb::sf::substreams::rpc::v2::response::Message;
use crate::pb::sf::substreams::rpc::v2::{BlockScopedData, BlockUndoSignal, Request, Response};
...
pub struct SubstreamsStream {
    stream: Pin<Box<dyn Stream<Item = anyhow::Result<BlockResponse>> + Send>>,
}
...
fn stream_blocks(
    ...
    start_block_num: i64,
    stop_block_num: u64,
) -> impl Stream<Item = anyhow::Result<BlockResponse>> {
    let mut start_cursor = cursor.unwrap_or_default();
    ...
    try_stream! {
        loop {
            ...
            match result {
                Ok(stream) => {
                    tracing::info!("Connected to substreams endpoint");

                    let mut encountered_error = false;
                    for await response in stream {
                        match process_substreams_response(response, &mut last_progress_report, &output_module_name) {
                            ...
                            BlockProcessedResult::TonicError(status) => {
                                ...
                                tracing::error!("Received tonic error {:#}", status);
                     ...
                     if !encountered_error {
                        tracing::info!("Stream completed, reached end block");
                        return
                    }
                }
                Err(err) => {
                    ...
                    tracing::error!("Unable to connect to endpoint: {:#}", err);
                    break;
                }
            }
            ...
            if let Some(duration) = backoff.next() {
                tracing::info!("Backing off for {:?}", duration);
                //TODO: exit after a max count has reached here
...
fn process_substreams_response(
    ...
    match response.message {
        ...
        None => {
            tracing::info!("Got None on substream message");
            BlockProcessedResult::Skip()
        }
...
impl Stream for SubstreamsStream {
    type Item = anyhow::Result<BlockResponse>;
